 ...
 */
public class MyDataBaseSong {
    private String TAG = MyDataBaseSong.class.getName();
    private final static String DB_NAME = "tangshisongci";
    private final static int DB_VERSION = 1;
    public Context context;
    private static MyDataBaseSong myDataBase;
    private SQLiteDatabase db;
    private SqliteBaseHelperSong sqliteBaseHelper;

    public static MyDataBaseSong getInstence() {
        if (null == myDataBase) {
            myDataBase = new MyDataBaseSong(MyApplication.getContext());
        }
        return myDataBase;
    }

    private MyDataBaseSong(Context context) {
        this.context = context;
        sqliteBaseHelper = new SqliteBaseHelperSong(context, DB_NAME, null, DB_VERSION);
    ...
    public void loadFromDBAsyn(final String sql,final String[] param,final MyModel myModel,final FindFinish findFinish) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Cursor cursor = db.rawQuery(sql, param);
                    final List<BaseModel> baseModels = myModel.fromCursor(cursor);
                    new Handler(Looper.getMainLooper())
                            .post(new Runnable() {
                                @Override
                                public void run() {
                                    findFinish.success(baseModels);
                                }
                            });
